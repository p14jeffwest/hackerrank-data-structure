...
public class Solution {
    ...
    // Rotating k slots right puts the last k elements in front of the first
    // n-k. Two things carry the solution:
    //
    //   1. k can be far larger than n, so reduce it with k % n first.
    //      Rotating one slot at a time k times is O(n*k) and cannot finish.
    //   2. After reduction the answer is two runs read in order, so every
    //      element is copied exactly once and nothing is ever shifted.
    //
    // Note that get(i) on an array-based list is O(1), which is what makes
    // this O(n). On a linked list the same code would be O(n^2).

    public static ListInterface<Integer> rotate(ListInterface<Integer> list, int k) {
        int n = list.size();
        k %= n;

        ListInterface<Integer> result = new Array_List<>(n);
        for (int i = n - k; i < n; i++) result.add(list.get(i));
        for (int i = 0; i < n - k; i++) result.add(list.get(i));
        return result;
    }
