...
public class Solution {
    ...
    // Boyer-Moore voting.
    //
    // Hold one candidate and a counter. Every element equal to the candidate
    // adds a vote, every other element cancels one. When the counter reaches
    // zero the candidate has been outvoted by the elements seen so far, so
    // the next element becomes the new candidate.
    //
    // Why the survivor is the answer: pair off each non-majority element with
    // one majority element and cancel both. The majority element occurs more
    // than n/2 times, so it cannot be exhausted -- some of it is always left
    // when the pairing runs out.
    //
    // O(n) time, O(1) extra space, and the list is only read.
